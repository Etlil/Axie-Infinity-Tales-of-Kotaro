...
export default class DungeonScene extends SceneBase {
  ...
  create() {
    ...
    pill(this, 193, 61, 'PUFFY’S LAGOON', { width: 261, size: 15 });
    ...
    STOPS.forEach((x, i) => {
      ...
      label(this, x, 482, i === 2 ? 'PUFFY' : `CLEARING ${i + 1}`, 13, '#527b61').setOrigin(0.5).setLetterSpacing(1);
      if (!passed) {
        if(i===2)fighter(this,x,354,'puffy',.66,'left').setCorrupted(true);
        else drawAxie(this,x,354,{kind:'mob',scale:.48});
      }
      if (current) {
