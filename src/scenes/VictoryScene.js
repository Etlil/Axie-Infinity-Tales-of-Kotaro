...
export default class VictoryScene extends SceneBase{
  ...
  create(){
    ...
    if(this.state.roomIndex===2){
      const momo=fighter(this,780,460,'momo',1.3,'left');
      if(this.state.result?.kind==='purify')momo.setAlpha(.55);
      else{const light=this.add.circle(780,432,100,0xffe8aa,.22);this.tweens.add({targets:light,scale:1.3,alpha:.05,duration:1800,yoyo:true,repeat:-1});}
    }
  }
  onCommand(action){
    if(action==='purify'&&this.state.result?.kind==='purify'&&this.state.amulet){
      this.session.rescue(bosses.momo);const rewards=this.session.completeStage(2);
      this.session.patch({result:{kind:'rescued',...rewards},message:'The amulet shines. Momo is coming home.'});this.scene.restart();
    }
  }
}
