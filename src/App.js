...
function AbilityCard({ card, index, disabled, command }) {
  return <button className={'ability-card card-' + card.kind} data-part={card.part}
    disabled={disabled} onClick={() => command('playCard', card.id)} aria-keyshortcuts={String(index + 1)}>
...
function Panel({ game, command, close, panel, fullscreen, showHelp }) {
  ...
  return <div className="modal-backdrop" onClick={close}><section ref={dialog} className={'panel panel-' + panel} role="dialog" aria-modal="true" aria-labelledby="panel-title" onClick={event => event.stopPropagation()}>
    ...
    {panel === 'help' && <><ol className="guide-list"><li><strong>Play an ability.</strong> Tap a Horn, Mouth, Back, or Tail card, or use 1 / 2 / 3 / 4. Any three abilities charge your ultimate (5).</li><li><strong>Watch, then dodge.</strong> The marked lane becomes dangerous in the final second. Tap Left, Center, or Right to move to a safe lane. Keyboard: A / S / D or arrows.</li><li><strong>Bring Atia back.</strong> The village gate opens your route. Claim Adventure Rank rewards at Buba’s tent. Switch companions from the Team menu.</li><li><strong>Use Buba’s amulet.</strong> Defeat the corrupted lagoon guardian, then choose “Use the amulet” to rescue Momo.</li></ol><p className="journal-note">Your story, companions, rewards, and rank save automatically in this browser. An unfinished encounter restarts from the village or Buba’s clearing after a reload.</p><p className="muted">Touch controls work in portrait and landscape. Landscape gives the village more room.</p></>}
    </div>
  </section></div>;
}
