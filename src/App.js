...
export default function App() {
  ...
  const puffyHome = game.rescued.some(resident=>resident.id==='puffy');
  ...
  return <main className={'game-app scene-' + game.scene + (story ? ' story-scene' : '') + (dodge ? ' is-dodging' : '') + (combat && ['PLAYER_FOCUS','PLAYER_TURN'].includes(game.phase) ? ' is-choosing' : '')} data-scene={game.scene} data-phase={game.phase} data-enemy={game.enemy?.id} style={{ '--village-image': 'url("/assets/atia/village.png")', '--kotaro-image': 'url("/assets/atia/kotaro-sheet.png")', '--buba-image': 'url("/assets/atia/buba-sheet.png")', '--arena-image': 'url("/assets/atia/' + (game.tutorial ? 'village.png' : game.roomIndex === 2 ? 'lagoon-arena.jpg' : 'forest-arena.jpg') + '")' }}>
    ...
    <WorldView scene={game.scene}><div className="game-mount" ref={mount} aria-label="Atia game world"/>
      {game.scene === 'village' && <div className="world-hotspots">
        ...
        {puffyHome && <button className="hotspot healer-hotspot" aria-label="Puffy’s healing spring" onClick={() => command('openPanel','healer')}><span className="hotspot-icon"><Icon name="heart"/></span><strong>Puffy</strong><small>Village healer · Free care</small></button>}
    ...
    {!game.loading && game.scene === 'village' && <>
      <button className="quest-card" aria-label="Open story quest" onClick={() => command('openPanel', 'journal')}><span className="quest-symbol"><Icon name={game.rescued.length ? 'check' : 'book'}/></span><span className="quest-copy"><small>STORY QUEST · CHAPTER 01</small><strong>{game.rescued.length ? 'A friend comes home' : 'A promise to keep'}</strong><span>{game.rescued.length ? 'Puffy rescued · +5% dodge time' : 'Find Puffy · ' + game.completedStages.length + ' / 3 cleared'}</span></span><Icon name="arrow" size={16}/></button>
      ...
      <nav className="village-dock" aria-label="Village menu">
        {puffyHome ? <button className="dock-item healer-dock" aria-label="Visit Puffy" onClick={() => command('openPanel','healer')}><span className="puffy-dock-portrait"><img src="/assets/atia/puffy-avatar.png" alt=""/></span><span>Puffy</span></button> : <button className="dock-item active" onClick={() => command('visitVillage')}><Icon name="home"/><span>Village</span></button>}
    ...
    {!game.loading && game.scene === 'map' && <>
      ...
      <section className="map-title"><button className="small-button" onClick={() => command('visitVillage')}><Icon name="back" size={17}/>Atia Village</button><p className="eyebrow">CHAPTER I · BEYOND THE GATE</p><h1>The road to Puffy</h1><p>One step deeper. One friend closer.</p></section>
      ...
      <section className="stage-detail"><div><span className="eyebrow">{game.selectedStage === 2 ? 'CORRUPTED GUARDIAN' : 'FOREST ENCOUNTER'} · STAGE 0{game.selectedStage + 1}</span><h2>{selected.location}</h2><p>{game.selectedStage === 2 ? 'Puffy waits beneath the shadows. Carry Buba’s light into the lagoon.' : game.selectedStage === 1 ? 'The old crossing flickers with nightmare light. Clear a path through.' : 'Thorns have claimed the old trail. Take the first step beyond Atia.'}</p><div className="stage-facts"><span><Icon name="sword" size={16}/>{selected.maxHP} HP</span><span><Icon name="spark" size={16}/>{game.completedStages.includes(game.selectedStage) ? 20 : selected.xp} XP</span><span>{game.completedStages.includes(game.selectedStage) ? 'Replay available' : 'First clear'}</span></div></div><GoldButton onClick={() => command('enterDungeon')}>{game.completedStages.includes(game.selectedStage) ? 'Replay encounter' : 'Enter encounter'}</GoldButton></section>
    </>}
    {!game.loading && combat && <>
    ...
    {!game.loading && game.scene === 'victory' && <section className="result-card"><span className="result-emblem">{game.roomIndex === 2 ? <img src="/assets/atia/puffy-avatar.png" alt="Puffy"/> : <Icon name={game.result?.kind === 'purify' ? 'spark' : 'check'} size={34}/>}</span><p className="eyebrow">{game.result?.kind === 'purify' ? 'THE AMULET IS GLOWING' : game.result?.kind === 'rescued' ? 'A LIGHT RETURNS TO ATIA' : 'THE PATH IS CLEAR'}</p><h1>{game.result?.kind === 'purify' ? 'There’s still a light inside.' : game.result?.kind === 'rescued' ? 'Welcome home, Puffy.' : 'A little further from fear.'}</h1><p>{game.result?.kind === 'purify' ? 'The nightmare is weakened. Use Buba’s amulet to bring Puffy back.' : game.result?.kind === 'rescued' ? 'The corruption fades. Puffy returns as Atia’s healer, offering free healing in the village and 5% more time to dodge.' : 'The next clearing is waiting. Your courage is helping Atia grow.'}</p>{game.result?.xp && <div className="result-rewards"><span><Icon name="spark"/>+{game.result.xp} XP</span><span><Icon name="coin"/>+{game.result.coins}</span><span>Rank {game.level}</span></div>}{game.result?.kind === 'purify' ? <GoldButton onClick={() => command('purify')}>Use the amulet</GoldButton> : <><GoldButton onClick={() => command(game.result?.kind === 'rescued' ? 'visitVillage' : 'returnMap')}>{game.result?.kind === 'rescued' ? 'Bring Puffy home' : 'Continue journey'}</GoldButton><button className="text-button" onClick={() => command('visitVillage')}>Return to Atia</button></>}</section>}
