...
function Panel({ game, command, close, panel, fullscreen, showHelp, showSettings, resetSave }) {
  ...
  return <div className="modal-backdrop" onClick={close}><section ref={dialog} className={'panel panel-' + panel} role="dialog" aria-modal="true" aria-labelledby="panel-title" onClick={event => event.stopPropagation()}>
    ...
    {panel === 'amulet' && <><div className="amulet-art"><Icon name="spark" size={72}/></div><blockquote>“It cannot mend everything. But it can bring someone back.”<cite>— Buba</cite></blockquote><p>Buba made this amulet from fragments of Atia’s old light. Weaken a corrupted guardian in battle, then use its light to reverse the corruption.</p><div className="journal-note"><Icon name="heart"/><div><strong>{game.rescued.length ? 'Puffy is home' : 'Your first rescue awaits'}</strong><p>{game.rescued.length ? 'Puffy heals you for free at the village spring. Puffy’s blessing also grants 5% more dodge time.' : 'Explore the Sunken Halls and reach Puffy’s chamber. Bring a villager back to Atia.'}</p></div></div></>}
...
export default function App() {
  ...
  return <main className={'game-app scene-' + game.scene + (game.dungeonRun ? ' in-dungeon' : '') + (story ? ' story-scene' : '') + (dodge ? ' is-dodging' : '') + (combat && ['PLAYER_FOCUS','PLAYER_TURN'].includes(game.phase) ? ' is-choosing' : '')} data-scene={game.scene} data-phase={game.phase} data-enemy={game.enemy?.id} style={{ '--village-image': 'url("/assets/atia/village.png")', '--kotaro-image': 'url("/assets/atia/kotaro-sheet.png")', '--buba-image': 'url("/assets/atia/buba-sheet.png")', '--arena-image': 'url("/assets/atia/' + (game.tutorial ? 'village.png' : game.roomIndex === 2 ? 'lagoon-arena.jpg' : 'forest-arena.jpg') + '")' }}>
