...
export default function MainMenu({game,command}){
  ...
  const root=useRef(null),page=game.menuPage||'home';
  ...
  return <section ref={root} className={'main-menu menu-page-'+page} aria-label="Main menu" style={{'--menu-image':'url("/assets/prologue/stone-table.png")'}}>
    <div className="menu-brand"><div className="menu-logo-panel"><GameLogo/></div><span className="menu-chapter">CHAPTER I · THE MOON'S PROMISE</span><p className="menu-tagline">A lost village. A promise to keep.</p></div>
    {page==='home'&&<nav className="title-actions" aria-label="Title menu">
      <span className="menu-nav-caption" aria-hidden="true">YOUR ADVENTURE AWAITS</span>
    ...
    {page==='slots'&&<div className="menu-sheet slot-sheet">
      <div className="menu-sheet-heading"><div><span className="eyebrow">CHOOSE YOUR JOURNEY</span><h2>Load game</h2></div>{back}</div>
    ...
    {page==='credits'&&<div className="menu-sheet credits-sheet"><div className="menu-sheet-heading"><h2>Made with a little light</h2>{back}</div><span className="eyebrow">AN AXIE FAN ADVENTURE</span><h3>Created by Etlil</h3><p>For Vibeathon 2026, and everyone helping Atia find its way home.</p><dl><dt>World & characters</dt><dd>Axie Infinity · Sky Mavis</dd><dt>Character & environment materials</dt><dd>Axie Origins Asset Kit and the Axie community</dd><dt>Town & Kotaro artwork</dt><dd>Artwork supplied by Etlil</dd><dt>Opening cutscene art</dt><dd>AI-generated layers, animated in Phaser</dd><dt>Game engine</dt><dd>Phaser · React</dd><dt>Typefaces</dt><dd>Changa One · Nunito · SIL Open Font License</dd></dl><p className="slot-footnote">An independent fan prototype. Not an official Sky Mavis release.</p></div>}
