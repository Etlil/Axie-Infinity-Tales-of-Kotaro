...
export function BubaConversation({game,command,paused}){
  ...
  if(line.type==='name')return <section className="story-choice name-card" aria-label="Introduce yourself">
    <img src="/assets/atia/buba-avatar.png" alt="Buba"/><div><small>BUBA</small><p>{line.text}</p>
    <form onSubmit={e=>{e.preventDefault();if(!paused&&draft.trim())command('namePlayer',draft);}}><label htmlFor="traveler-name">Your name</label><div className="name-entry"><input id="traveler-name" value={draft} maxLength={20} autoComplete="off" autoFocus onChange={e=>setDraft(e.target.value)} placeholder="Kotaro" disabled={paused}/><button className="gold-button" disabled={paused||!draft.trim()} type="submit">Confirm name</button></div><small>Up to 20 characters · Saved with this adventure</small></form></div>
  </section>;
