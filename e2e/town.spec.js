...
test('walk the town, collide with the well, interact with Buba, and follow the gate to the level map',async({page})=>{
  ...
  await page.goto('/');await enterAdventure(page);await expect(page.locator('.town-bottom')).toHaveAttribute('data-tile-y','29');
  await page.keyboard.down('ArrowUp');await expect(page.locator('.town-bottom')).toHaveAttribute('data-tile-y','16');await page.waitForTimeout(400);await page.keyboard.up('ArrowUp');
  await expect(page.locator('.town-bottom')).toHaveAttribute('data-tile-y','16');
  await page.keyboard.press('e');await expect(page.getByRole('dialog',{name:'Save fountain'})).toBeVisible();await page.keyboard.press('Escape');
  await page.getByRole('button',{name:'Rewards',exact:true}).click();
  await expect(page.getByRole('dialog',{name:'Buba’s tent'})).toBeVisible();
  await expect(page.locator('.town-bottom')).toHaveAttribute('data-tile-x','11');
  ...
  await expect(page.locator('.town-bottom')).toHaveAttribute('data-tile-x','37');await expect(page.locator('.town-bottom')).toHaveAttribute('data-tile-y','7');
...
for(const viewport of [{width:568,height:320},{width:390,height:844}])test(`town touch controls and destinations at ${viewport.width}x${viewport.height}`,async({browser})=>{
  ...
  try{
    ...
    await touch.send('Input.dispatchTouchEvent',{type:'touchStart',touchPoints:[{x:up.x+up.width/2,y:up.y+up.height/2}]});await expect(page.locator('.town-bottom')).toHaveAttribute('data-tile-y','16');
    ...
    await page.waitForTimeout(400);await expect(page.locator('.town-bottom')).toHaveAttribute('data-tile-y','16');
    ...
    await expect(page.locator('.town-bottom')).toHaveAttribute('data-tile-x','34');await page.getByRole('button',{name:'Close panel'}).tap();
